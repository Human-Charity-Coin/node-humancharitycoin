# A Node.js HumanCharityCoin Client!

![HumanCharityCoin](https://raw.githubusercontent.com/Human-Charity-Coin/node-humancharitycoin/master/node-humancharitycoin.png)

node-humancharitycoin is a HumanCharityCoin client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with HumanCharityCoin. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with HumanCharityCoin integration.
* Prevent would-be HumanCharityCoin web developers worrying whether a Bitcoin client will work out of the box.
* Promote Node.js development of HumanCharityCoin web apps.
* Identify and address any incompatibilities with the HumanCharityCoin and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [humancharitycoind](https://github.com/Human-Charity-Coin/HCC-wallet) to connect with.

Then, install the `node-humancharitycoin` NPM package.

`npm install node-humancharitycoin`

## Examples

Some code examples follow below, but for more complete examples, see [these snippets](https://github.com/Human-Charity-Coin/humancharitycoin-code-snippets), or this [wallet app](https://github.com/Human-Charity-Coin/hcclet) which was created to to test this module.

```js
var humancharitycoin = require('node-humancharitycoin')()

humancharitycoin.auth('myusername', 'mypassword')

humancharitycoin.getDifficulty(function() {
...
```js
humancharitycoin.getNewAddress(function(err, address) {
...
```js
humancharitycoin.exec('getNewAddress')

humancharitycoin.exec('getbalance', function(err, balance) {
...
```js
humancharitycoin.set('host', '127.0.0.1')
```
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a humancharitycoin address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [humancharitycoinaddress] </td>
<td> Reveals the private key corresponding to <humancharitycoinaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current humancharitycoin address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether humancharitycoind is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new humancharitycoin address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [humancharitycoinaddress] [minconf=1] </td>
<td> Returns the total amount received by <humancharitycoinaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<tr>
<td> importprivkey </td>
<td> [humancharitycoinprivkey] [label] </td>
...
<p>To get a list of accounts on the system, execute humancharitycoind listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [tohumancharitycoinaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<tr>
<td> signmessage </td>
<td> [humancharitycoinaddress] [message] </td>
...
<td> stop </td>
<td> </td>
<td> Stop humancharitycoin server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [humancharitycoinaddress] </td>
<td> Return information about [humancharitycoinaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [humancharitycoinaddress] [signature] [message] </td>
...
var humancharitycoin = require('humancharitycoin')({
    user:'user'
})

humancharitycoin.set('pass', 'somn')
humancharitycoin.set({port:22555})
...
You may pass an optional function `passphrasecallback` to the `node-humancharitycoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `humancharitycoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `humancharitycoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `humancharitycoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/humancharitycoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/humancharitycoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `humancharitycoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-humancharitycoin is secured!
...
nodeunit test/test-node-humancharitycoin.js
```
