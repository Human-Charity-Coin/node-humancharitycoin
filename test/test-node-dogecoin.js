...
var humancharitycoin = require('../lib/humancharitycoin')(options);
...
exports.get = function (test) {
	...
	for(var propt in options){
		test.deepEqual(humancharitycoin.get(''+options_keys[idx]), options[''+propt]);
		idx ++;
	}
...
exports.set = function (test) {
	...
	for(var propt in new_options){
		humancharitycoin.set(''+options_keys[idx], new_options[ ''+options_keys[idx] ]);
		test.deepEqual(humancharitycoin.get(''+options_keys[idx]), new_options[''+propt]);
		idx ++;
	}
	test.done();
}
...
exports.commands_noAuth = {		
	...
	// 			or add "gen=1" to the bottom of your humancharitycoin.conf file
	...
	getGenerate: function(test){
		test.expect(2);
		humancharitycoin.setGenerate(true,1);	
		test.equal(humancharitycoin.getGenerate(), true);

		humancharitycoin.setGenerate(false,1);	
		test.equal(humancharitycoin.getGenerate(), false);
		test.done();
	},
	getreceived_: function(test){
		var amount= 0.0001;
		//humancharitycoin.setAccount()
		sendfrom("testnet_user", humancharitycoin.getaccountaddress('testnet_user'),amount, function(err,addr){
			test.equal(getreceivedbyaccount('testnet_user', amount);
			test.equal( getreceivedbyaddress( humancharitycoin.getaccountaddress('testnet_user') ), amount);
			test.done();
...
exports.commands_Auth = {		
	...
	setUp: function () {
		humancharitycoin.auth('testnet_user', 'testnet_pass');
	}
